...
_blackVehicles = ["B_Heli_Light_01_armed_F","O_Heli_Transport_04_F"];			// black skin
...
if (_t in _pods) then {
	_u setObjectTextureGlobal [0,"#(argb,8,8,3)color(0,0,0,0.6)"];
	_u setObjectTextureGlobal [1,"#(argb,8,8,3)color(0,0,0,0.6)"];
};

if (_t in _podsAction) then {
...
//===== MHQ - R3F
if (_t in _mhq) then {
	_u setVariable ["R3F_LOG_disabled", false];
	_u addAction ["<t color='#ff1111'>VAS</t>","scripts\VAS\open.sqf",[],10,true,true,'((vehicle player) == player) && ((player distance _target) < 5)'];
...
if (_t in _buzzard) then {
	TVS= [_u,"Bo_GBU12_LGB_MI10"] execvm "scripts\vehicle\TVS\scripts\init.sqf";	
};
