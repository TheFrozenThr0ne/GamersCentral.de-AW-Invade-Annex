...
for "_x" from 1 to PARAMS_AAPatrolSecondary do {
	_aaGroup = createGroup east;
	_randomPos = [[[getMarkerPos currentA, (PARAMS_AOSizeSecondary / 2)],[]],["water","out"]] call BIS_fnc_randomPos;
	...
	[_aaGroup, getMarkerPos currentA, 500] call BIS_fnc_taskPatrol;
	_aa lock 3;
	_aaGroup setVariable ["VCOM_Unit_AIWarnDistance",200,false];
	_aaGroup setVariable ["NOPATHING",1,false];
	
	_wp = _aaGroup addWaypoint [getMarkerPos currentA, 0, 750];
...
for "_x" from 1 to PARAMS_GroupPatrolSecondary do {
	...
	[_patrolGroup, getMarkerPos currentA, 600] call BIS_fnc_taskPatrol;
	_patrolGroup setVariable ["VCOM_Unit_AIWarnDistance",200,false];
	_patrolGroup setVariable ["NOPATHING",1,false];	
	
	_wp = _patrolGroup addWaypoint [getMarkerPos currentA, 0, 750];
...
for "_x" from 1 to PARAMS_StaticMG do {
	...
	_randomPos = [getMarkerPos currentA, 200, 10, 10] call BIS_fnc_findOverwatch;
...
for "_x" from 1 to PARAMS_StaticMGSecondary do {
	_staticGroup = createGroup east;
...
for "_x" from 1 to PARAMS_OverwatchSecondary do {
	...
	_randomPos = [getMarkerPos currentA, 650, 50, 10] call BIS_fnc_findOverwatch;
	...
	_wp = _overwatchGroup addWaypoint [getMarkerPos currentA, 0, 750];
...
for "_x" from 0 to 1 do {
	...
	_randomPos = [[[getMarkerPos currentA, PARAMS_AOSizeSecondary],[]],["water","out"]] call BIS_fnc_randomPos;
	...
	[_AOmrapGroup, getMarkerPos currentA, 600] call BIS_fnc_taskPatrol;
	_AOmrap lock 3;
	...
	_wp = _AOmrapGroup addWaypoint [getMarkerPos currentA, 0, 750];
...
for "_x" from 0 to (1 + (random 1)) do {
	...
	[_AOvehGroup, getMarkerPos currentA, 400] call BIS_fnc_taskPatrol;
	_AOveh lock 3;
	_AOvehGroup setVariable ["VCOM_Unit_AIWarnDistance",200,false];
	_AOvehGroup setVariable ["NOPATHING",1,false];	
	
	_wp = _AOvehGroup addWaypoint [getMarkerPos currentA, 0, 750];
...
if((random 10 <= PARAMS_AirPatrolSecondary)) then {
	_airGroup = createGroup east;
	_randomPos = [[[getMarkerPos currentA, PARAMS_AOSizeSecondary],_dtOthers],["water","out"]] call BIS_fnc_randomPos;
	...
	[_airGroup, getMarkerPos currentA, 800] call BIS_fnc_taskPatrol;
...
for "_x" from 1 to PARAMS_SniperTeamsPatrolSecondary do {
	_sniperGroup = createGroup east;
	_randomPos = [getMarkerPos currentA, 400, 100, 10] call BIS_fnc_findOverwatch;
...
if (PARAMS_EnemiesInBuildings != 0) then {
	...
	if ((count _town) > 6) then {
		...
		_toSpawn = [];
		for [{_i = 0}, {_i < PARAMS_EnemiesInBuildingsSecondary}, {_i = _i + 1}] do {
...
} forEach (getMarkerPos currentA nearObjects ["House", 800]);
