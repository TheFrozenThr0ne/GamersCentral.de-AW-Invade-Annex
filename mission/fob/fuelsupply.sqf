...
|	Coded for AhoyWorld.
*/
depotTruck = "B_Truck_01_fuel_F" createVehicle getMarkerPos "FOB_Depot";
...
_briefing = "<t align='center'><t size='2.2'>New Supply Mission</t><br/><t size='1.5' color='#00B2EE'>Fuel</t><br/>____________________<br/>We have provided a truck with some fuel for the FOB<br/><br/>Get the supplies to the FOB and watch out for OPFOR ambushes</t>";
...
if (depotTruck distance (getMarkerPos "FOB_Marker") < 21) then {
	_fuelBox = "B_Slingload_01_Fuel_F" createVehicle (getMarkerPos fuelPad1);
	_vicDir1 = (markerDir fuelPad1);
	_fuelBox setDir _vicDir1;
...
if (!(Fob_missions)) then {
	...
	_briefing = "<t align='center'><t size='2.2'>FOB Mission</t><br/><t size='1.5' color='#00B2EE'>Fuel</t><br/>____________________<br/>Operations in the area have ceased.<br/><br/>The resources are no longer needed.</t>";
	[_briefing] remoteExec ["AW_fnc_globalHint",0,false];
	[hqSideChat] remoteExec ["AW_fnc_globalSideChat",0,false];
};
