...
while { count _targetArray > 0 } do {
	...
	if ((_flatPos distance (getMarkerPos "radioMarker")) > 800) then
		{
			_accepted = true;
	};
	};
	...
	//_marker1 = createMarker ["cache1",getpos radioTower];
	...
	radarTower = "Land_Radar_Small_F" createVehicle _flatPosa;
	waitUntil { sleep 0.5; alive radarTower };
	{ _x setMarkerPos _roughPosa; } forEach ["radarMarker", "radarCircle"];
	_spawnRandomisationcache=180;
	_spwnposNewcache = [(getMarkerPos "radarMarker"),random _spawnRandomisationcache,random 360] call BIS_fnc_relPos;
	radarTower setpos _spwnposNewcache;
	
	
	
	//{ _x setMarkerPos _roughPos getPos radarTower; } forEach ["cache2", "cache2Circle"];
	//_marker2 = createMarker  ["cache2",getpos radarTower];
	...
	//"cache2" setMarkerText "Cache";
	{
		...
		_x addCuratorEditableObjects [[radarTower], false];
	} foreach allCurators;
	publicVariable "radioTower";
	publicVariable "radarTower";
	if (isServer) then {
		if (isNil "HeadlessVariable") then {
				
			_enemiesArray = [currentAO] call QS_fnc_AOenemyRadar;
	...
	showNotification = ["NewSub", "Destroy the enemy Radiotower."]; publicVariable "showNotification";	
	...
	_objs = [radioTower, radarTower];

	waituntil {sleep 0.3;{alive _x} count _objs == 1;};
	//hint"1/2 caches destroyed";
	
	_cacheDown = "<t align='center' size='2.2'>1/2</t><br/><t size='1.5' color='#08b000' align='center'>DESTROYED</t><br/>____________________<br/>One of the enemy cache has been destroyed! One Cache left, seek and destroy it!<br/>";
	GlobalHint = _cacheDown; hint parseText GlobalHint; publicVariable "GlobalHint";
	
	if (!alive radioTower) then {{ _x setMarkerPos [-10000,-10000,-10000]; } forEach ["radioMarker", "radioCircle"];} else {{ _x setMarkerPos [-10000,-10000,-10000]; } forEach ["radarMarker", "radarCircle"];};
	waituntil {sleep 0.3;{alive _x} count _objs == 0;};
	if (!alive radioTower) then {{ _x setMarkerPos [-10000,-10000,-10000]; } forEach ["radioMarker", "radioCircle"];} else {{ _x setMarkerPos [-10000,-10000,-10000]; } forEach ["radarMarker", "radarCircle"];};
	if (!alive radarTower) then {{ _x setMarkerPos [-10000,-10000,-10000]; } forEach ["radarMarker", "radarCircle"];} else {{ _x setMarkerPos [-10000,-10000,-10000]; } forEach ["radioMarker", "radioCircle"];};
