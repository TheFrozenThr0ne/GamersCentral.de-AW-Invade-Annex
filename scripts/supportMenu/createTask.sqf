private ["_caller","_screenTarget","_taskName","_driver","_type","_taskLetterArray","_taskLetter","_taskLetter2","_taskNumber","_taskNumber2","_taskTaker","_taskState","_k","_l"];
_caller = _this select 1;
_screenTarget = ((_this select 3) select 0);
_driver = (_this select 3) select 1;
...
if (_type == "Attack") then {
    [_taskName, _caller, _tasktaker, _screenTarget] call lampcreateTaskAttackAll;
...
if (_type == "Transport") then {
    [_taskName, _caller, _taskTaker, _screenTarget] call lampcreateTaskTransportAll;
...
waitUntil {sleep 1; _tasktaker distance _screenTarget < 500 || !alive _caller || "Failed" == _taskName call SHK_Taskmaster_getState || "Succeeded" == _taskName call SHK_Taskmaster_getState || "Canceled" == _taskName call SHK_Taskmaster_getState || !alive _tasktaker};

if (_type == "Attack") then {
    smokeRed = "SmokeShellRed" createVehicle _screenTarget;
    smokeGreen = "SmokeShellGreen" createVehicle (getPos _caller);
};
if (_type == "Transport") then {
    smokeYellow = "SmokeShellYellow" createVehicle _screenTarget;
};
