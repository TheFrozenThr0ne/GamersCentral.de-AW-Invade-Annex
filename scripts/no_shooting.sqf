...
_dis = 1000;                             // distance from area safe zone starts
...
if ((_zone1 distance _unit > _dis) or (_zone2 distance _unit > _dis)) then {        //check if unit is in zone when script starts
   ...
   _inArea = true;
   _eh1 = _unit addEventHandler ["fired", {deleteVehicle (_this select 6);titleText ["This area is currently a safe zone. Operation of arms is prohibited!","PLAIN"];}];
};
