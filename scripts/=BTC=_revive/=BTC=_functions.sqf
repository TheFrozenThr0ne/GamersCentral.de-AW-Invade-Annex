...
fnc_vas_jip = {
	private ["_veh"];
	_veh = _this select 0;
	_veh addAction ["<t color='#ff1111'>VAS</t>","scripts\VAS\open.sqf",[],10,true,true,'((vehicle player) == player) && ((player distance _target) < 5)'];
	_veh setVariable ["R3F_LOG_disabled", false];
	_veh addAction ["Quick Gear Save",QS_fnc_saveInventory,[],7,true,true,'((vehicle player) == player) && ((player distance _target) < 5)'];
	_veh addAction ["View Distance Settings",TAWVD_fnc_openTAWVD,[],-98,false,false,"",''];
};
...
BTC_vehicle_mobile_respawn = {
	...
	_veh setpos _pos;
	nul = [_veh, 7500, west, "LIGHT"] execVM "R3F_LOG\USER_FUNCT\init_creation_factory.sqf";
	_veh setVariable ["R3F_LOG_disabled", false];
	0 = [_veh] execVM "scripts\VAserver.sqf";
	[[_veh],"fnc_vas_jip",true,true] spawn BIS_fnc_MP;
	{
