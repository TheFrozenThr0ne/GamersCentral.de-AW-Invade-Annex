...
_checkThread = {
	{
		...
		if(!isNil "_var") then {
			...
			[[name player,getPlayerUID player,format["Variable: %1",_x]],"SPY_fnc_cookieJar",false,false] spawn BIS_fnc_MP;
			[[name player,format["Variable: %1",_x]],"SPY_fnc_notifyAdmins",true,false] spawn BIS_fnc_MP;
