...
if("DevCon" in _cfgPatches) then {
	[[name player,getPlayerUID player,"DevCon"],"SPY_fnc_cookieJar",false,false] spawn BIS_fnc_MP;
	[[name player,"DevCon"],"SPY_fnc_notifyAdmins",true,false] spawn BIS_fnc_MP;
...
/*[] spawn {
	while {true} do {
		if((unitRecoilCoefficient player) < 1) then {
			[[name player,getPlayerUID player,"No_recoil_hack"],"SPY_fnc_cookieJar",false,false] spawn BIS_fnc_MP;
			[[name player,"No recoil hack"],"SPY_fnc_notifyAdmins",true,false] spawn BIS_fnc_MP;
